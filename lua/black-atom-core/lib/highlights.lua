...
---@return BlackAtomCore.HighlightDefinition
...
function M.building_error_notification(message)
    local notification_opts = {
        title = "Black Atom Core - Highlights Error",
...
function M.build_highlights_map(colors, config)
    ...
    local highlight_modules = require("black-atom-core.lib.files").get_highlight_modules(
        "lua/black-atom-core/highlights",
    ...
    if config.debug then
        require("black-atom-core.lib.debug").write_debug_highlights_file(highlight_modules, highlights_map)
    end
